...
Translation: [Chinese translation of the sentence]""",
    ...
    "zh": {
        "name": "中文",
        "prompt_template": """你是一个中文（文言文）教学助手。请为中文词语"{word}"（意思：{meaning}）生成一句文言文风格的例句，要求尽量引用或参照中国著名古籍（如《论语》《诗经》《史记》《左传》《庄子》《孟子》《韩非子》《楚辞》等）的表达风格或名句，但不要直接复制整段原文。

    【必须遵守的要求】：
    1. 例句必须使用文言文风格（文言短句、古典词汇与句式），不要使用现代白话句式。
    2. 例句必须包含原词"{word}"（词形如有需要可做适当古汉语化处理，但请保留原词可辨识性）。
    3. 若能自然地引用或暗示著名古籍语句，请在句末用括号标注可能的典故或出处（如有），如无可省略该注。
    4. 例句应简洁（不超过一句话），并适合用于词汇学习场景。
    5. 请同时提供一句现代汉语白话译文，帮助理解。
